...
!--  for refitting with the lineariyed coordinates later 
 ...
 !
  ...
  double precision :: b1,b3,b4,b5,theta1,theta2,theta3,y(NDEG),theta12,theta23,theta13,theta1e,theta2e,theta3e,SAeq,SBeq,Sa,Sb
...
!ahh3 = 240.0+tbar+1.0/6.0*sqrt(6.0)*A1-1.0/6.0*sqrt(2.0)*A2  !12
  ...
  !
  xi(10)   = ( 2.0d0*theta23 - theta13 - theta12 )/sqrt(6.0d0) !- SAeq
  xi(11)   = (                 theta13 - theta12 )/sqrt(2.0d0) !- SBeq
  ! xi(10)   = ( 2.0d0*theta12 - theta13 - theta23 )/sqrt(6.0d0) - SAeq
  ! xi(11)   = (                 theta23 - theta13 )/sqrt(2.0d0) - SBeq 
  !
  ! write(*, '(A, F20.10, F20.10, F20.10, F20.10, F20.10, F20.10, F20.10, F20.10, F20.10)'), "point ", theta2, theta3, theta12, theta13, theta23, Sa, Sb, xi(10), xi(11)
  !
  ! write(*, '(A, F20.10, F20.10)'), "point ", xi(10)/deg, xi(11)/deg
  !
  tau = (theta1 + theta2 + theta3-2.0d0*pi)/3.0d0
  !
  ! test = xi(3)*cos(tau + 2.0d0*pi/3.0d0) + xi(3)*cos(tau - 2.0d0*pi/3.0d0) + xi(4)*cos(tau) + xi(4)*cos(tau - 2.0d0*pi/3.0d0) + xi(5)*cos(tau) + xi(5)*cos(tau + 2.0d0*pi/3.0d0)
  ! write(*, '(A, F20.10, F20.10, F20.10, F20.10, F20.10)') "torsion", local(3), local(4), local(5), tau, test
  !
  xi(12) = tau
  !
